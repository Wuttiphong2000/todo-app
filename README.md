...
A full-stack productivity app with todos, habits, focus timer, and a calendar.

**Live demo:** https://independent-nature-production-12ef.up.railway.app/

> Try it instantly in **Guest Mode** — no account needed. Data stays in your browser.
...
| Layer | Technology |
|-------|-----------|
| Backend | Node.js · Express · TypeScript · PostgreSQL (pg) · Zod · JWT |
...
**https://independent-nature-production-12ef.up.railway.app**

Two Railway services share one project, each built from its subdirectory via Dockerfile:

| Service | Root Directory | Key env vars |
|---------|---------------|-------------|
| backend | `claude-todo-backend` | `JWT_SECRET`, `DATABASE_URL` (Railway PostgreSQL plugin), `CLIENT_URL` |
| frontend | `claude-todo-frontend-ts` | `PORT=80`, `BACKEND_URL=https://<backend>.up.railway.app` |

PostgreSQL is provided by Railway's PostgreSQL plugin; `DATABASE_URL` is injected automatically.
...
Open **http://localhost**. nginx serves the SPA and reverse-proxies `/api/` to the backend container. PostgreSQL runs as a separate container with data persisted in a named Docker volume (`postgres_data`).
...
Copy `.env.example` to `claude-todo-backend/.env` and fill in `JWT_SECRET` and `DATABASE_URL`.
...
│   │   ├── db/                 # pg pool + versioned migrations (v1–v6)
...
Backend tests require a PostgreSQL `DATABASE_URL` pointed at a test database.
...
Six versioned migrations (v1–v6) create and extend these tables:
...
Click **Continue as Guest** on the login page to use the full app without an account. All data is stored in `localStorage` only — nothing is sent to the server. Use **Export** to save your data before clearing browser storage.
